...
using MiningCore.Blockchain.Commercium.Configuration;
using MiningCore.Blockchain.Commercium.DaemonInterface;
...
using NLog;
using Block = MiningCore.Blockchain.Commercium.DaemonInterface.Block;
using Transaction = MiningCore.Blockchain.Commercium.DaemonInterface.Transaction;

namespace MiningCore.Blockchain.Commercium
{
    public class CommerciumJobManager: JobManagerBase<CommerciumJob>
    {
        ...
        private CommerciumPoolConfigExtra _extraPoolConfig;
        private CommerciumPoolPaymentProcessingConfigExtra _extraPoolPaymentProcessingConfig;
        private readonly List<CommerciumJob> _validJobs = new List<CommerciumJob>();
        ...
        protected virtual DaemonResponse<CommerciumGetWork> GetWorkFromJson(string json)
        {
            logger.LogInvoke(LogCat);

            var result = JsonConvert.DeserializeObject<JsonRpcResponse>(json);

            return new DaemonResponse<CommerciumGetWork>
            {
                Response = result.ResultAs<CommerciumGetWork>(),
            };
        }
        ...
        protected override string LogCat => "Commercium Job Manager";
        ...
        protected override async Task EnsureDaemonsSynchedAsync(CancellationToken ct)
        {
            ...
            while(true)
            {
                var responses = await _daemon.ExecuteCmdAllAsync<CommerciumGetWork>(BitcoinCommands.GetWork);
